...
#### Step 3.3: Update the Reset Scripts

Finally, update the reset scripts to remove the now-obsolete Alembic deletion commands.

**File to Edit: `reset.bat` (Full File Content)**
```bat
@echo off
...
echo.
echo   - The 'venv' directory will be deleted.
echo   - The '.env' file will be deleted.
echo   - The '.setup_state' file will be deleted.
echo   - The 'ollama_proxy.db' file will be deleted.
echo.
echo THIS OPERATION IS IRREVERSIBLE.
echo.
set "CONFIRMATION="
set /p CONFIRMATION="To confirm, please type 'reset now': "
if /i not "!CONFIRMATION!"=="reset now" (
    echo. & echo Reset cancelled. & pause & exit /b 1
)

echo. & echo Proceeding with reset... & echo.

echo 1. Deleting Python virtual environment ('venv')...
if exist "venv" ( rmdir /s /q "venv" & echo    - Done. ) else ( echo    - Not found. )

echo 2. Deleting configuration file ('.env')...
if exist ".env" ( del ".env" & echo    - Done. ) else ( echo    - Not found. )

echo 3. Deleting setup state file ('.setup_state')...
if exist ".setup_state" ( del ".setup_state" & echo    - Done. ) else ( echo    - Not found. )

echo 4. Deleting database file ('ollama_proxy.db')...
if exist "ollama_proxy.db" ( del "ollama_proxy.db*" 2>nul & echo    - Done. ) else ( echo    - Not found. )

echo 5. Cleaning up Python cache files...
...
echo #                         RESET COMPLETE                            #
echo #####################################################################
echo. & echo You can now run 'run_windows.bat' for a fresh installation. & echo.
pause